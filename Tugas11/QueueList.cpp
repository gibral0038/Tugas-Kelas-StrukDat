...
void createElmt(Pointer& pBaru) {
    ...
    cout << "Nama       : "; cin.ignore(); getline(cin, pBaru->info.nama);
    do {
        cout << "Golongan (1-3) : "; cin >> pBaru->info.gol;
        if (pBaru->info.gol < 1 || pBaru->info.gol > 3) {
            cout << "Golongan tidak valid!" << endl;
        }
    } while (pBaru->info.gol < 1 || pBaru->info.gol > 3);
    pBaru->next = nullptr;
}
