/*  Nama program    : d_PriorityQueuePegawai
    ...
    Deskripsi       : Priority Queue Pegawai Menggunakan Library Queue
******************************************************/
...
struct Perbandingan{
    bool operator()(const Pegawai& a, const Pegawai& b) {
        return a.gol < b.gol; 
    }
};

typedef priority_queue<Pegawai, deque<Pegawai>, Perbandingan> PriorityQueuePegawai;
...
void cetakPriorityQueue(PriorityQueuePegawai daftar){
    if (daftar.empty()){
        cout << "Queue kosong\n";
    } else {
        int count = 0;
...
void menu(PriorityQueuePegawai& PriorityQueuePegawai){
    int pilih;
    do {
        display();
        cin >> pilih;
        ...
        case 1 : {
            Pegawai baru;
            baru = createElement();
            PriorityQueuePegawai.push(baru);
            cout << "Pegawai " << baru.nama << " berhasil ditambahkan\n";
            break;
        }
        case 2 : {
            Pegawai hasil;
            if (PriorityQueuePegawai.empty()){
                cout << "Queue kosong\n";
            } else {
                hasil = PriorityQueuePegawai.top();
                PriorityQueuePegawai.pop();
                cout << "Pegawai " << hasil.nama << " berhasil dihapus\n";
            }
            break;
        }
        case 3 : {
            cetakPriorityQueue(PriorityQueuePegawai);
            break;
        }
        case 4 :
